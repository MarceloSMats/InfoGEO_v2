...
def _load_macro_rta_gdf():
    ...
    try:
        ...
        MACRO_RTA_GDF = gdf
        logger.info(f"✅ MACRO_RTA shapefile carregado: {len(gdf)} regiões, CRS: {gdf.crs}")
        ...
        if 'CD_RTA' in gdf.columns:
            ...
            logger.info(f"   ℹ️ CD_RTA será usado como CD_MICR_GEO no Excel")
        else:
            logger.error(f"   ❌ Campo CD_RTA não encontrado! Colunas: {gdf.columns.tolist()}")
...
def _get_cd_rta_from_centroid(centroid_point_wgs84: Point):
    ...
    """
    try:
        logger.info(f"🌍 Centroide recebido: Lat={centroid_point_wgs84.y:.6f}, Lon={centroid_point_wgs84.x:.6f}")
        ...
        logger.info(f"📍 MACRO_RTA carregado: {len(gdf_macro)} regiões, CRS: {gdf_macro.crs}")

        if gdf_macro.crs.to_string() != 'EPSG:4326':
            logger.info(f"🔄 Transformando centroide de EPSG:4326 para {gdf_macro.crs}")
            transformer = Transformer.from_crs('EPSG:4326', gdf_macro.crs, always_xy=True)
            centroid_transformed = shapely_transform(transformer.transform, centroid_point_wgs84)
            logger.info(f"   Centroide transformado para: {centroid_transformed}")
        else:
            centroid_transformed = centroid_point_wgs84
            logger.info(f"✅ CRS já é EPSG:4326, sem necessidade de transformação")

        logger.info(f"🔍 Buscando região que contém o centroide...")
        matches = gdf_macro[gdf_macro.geometry.contains(centroid_transformed)]

        if matches.empty:
            logger.warning(f"⚠️ Nenhuma região contém exatamente o ponto. Tentando intersects...")
            matches = gdf_macro[gdf_macro.geometry.intersects(centroid_transformed)]

        if matches.empty:
            logger.error(f"❌ Centroide ({centroid_point_wgs84.x:.6f}, {centroid_point_wgs84.y:.6f}) não encontrado em nenhuma região MACRO_RTA")
        ...
        logger.info(f"   Este valor será usado como CD_MICR_GEO para buscar as notas agronômicas")
...
def _get_nota_from_micro_classe(cd_micr_geo: int, cls_num: int):
    ...
    try:
        ...
        if df is None or df.empty:
            logger.warning(f"⚠️ DataFrame de micro_classes vazio ou não carregado")
            return None

        logger.info(f"🔍 Buscando nota: CD_MICR_GEO={cd_micr_geo} (tipo: {type(cd_micr_geo).__name__}), COD_DN={cls_num}")
        ...
        matches = df[(df['CD_MICR_GEO'] == cd_micr_geo_int) & (df['COD_DN'] == cls_num_int)]

        if matches.empty:
            logger.warning(f"⚠️ Nenhuma linha encontrada para CD_MICR_GEO={cd_micr_geo_int}, COD_DN={cls_num_int}")
            logger.info(f"   Valores únicos de CD_MICR_GEO no Excel: {sorted(df['CD_MICR_GEO'].unique())[:20]}")
            logger.info(f"   Valores únicos de COD_DN no Excel: {sorted(df['COD_DN'].unique())}")
            return None

        nota_val = matches.iloc[0]['NOTA_AGRONOMICA']

        if nota_val is None or (isinstance(nota_val, float) and nota_val != nota_val):
            logger.warning(f"⚠️ NOTA_AGRONOMICA é None/NaN para CD_MICR_GEO={cd_micr_geo_int}, COD_DN={cls_num_int}")
            return None

        try:
            nota_float = float(str(nota_val).replace(',', '.'))
            logger.info(f"✅ Nota encontrada: CD_MICR_GEO={cd_micr_geo_int}, COD_DN={cls_num_int}, NOTA={nota_float}")
            return nota_float
        except Exception as e:
...
def _get_quadrante_info_from_centroid(centroid_point_wgs84: Point):
    ...
    try:
        ...
        try:
            logger.info(f"\n{'─'*80}")
            logger.info(f"🔍 ETAPA 2/3: BUSCANDO CD_RTA (Código da Microregião)")
            logger.info(f"{'─'*80}")
            logger.info(f"   Método: Cruzamento espacial do centroide com MACRO_RTA.shp")
            logger.info(f"   Centroide: Lat={centroid_point_wgs84.y:.6f}, Lon={centroid_point_wgs84.x:.6f}")
            ...
            if cd_rta is not None:
                ...
                logger.info(f"   ℹ️ CD_RTA = CD_MICR_GEO (mesmo código de microregião)")
                logger.info(f"   Será usado para buscar notas no Excel CD_MICRO_CLASSES.xlsx")
                logger.info(f"{'─'*80}\n")
            else:
                logger.error(f"❌ FALHA CRÍTICA: CD_RTA não foi encontrado!")
                logger.error(f"   CONSEQUÊNCIA: Notas agronômicas usarão fallback 1.0")
                logger.error(f"{'─'*80}\n")
...
def calculate_valoracao(relatorio, centroid_point, valor_quadrante_result):
    ...
    # Extrair CD_MICR_GEO
    logger.info(f"\n{'='*80}")
    logger.info(f"🔍 ETAPA 3/3: PREPARANDO DADOS PARA CÁLCULO DE VALORAÇÃO")
    logger.info(f"{'='*80}")

    cd_micr_geo = None
    if isinstance(quad_attrs, dict):
        cd_micr_geo = (
            quad_attrs.get('CD_RTA') or
            quad_attrs.get('CD_MICR_GEO') or
            quad_attrs.get('cd_micr_geo') or
            quad_attrs.get('CD_MICR_GE') or
            quad_attrs.get('cd_micr_ge') or
            quad_attrs.get('CD_MICR__1')
        )

    logger.info(f"📊 Atributos disponíveis no quadrante: {list(quad_attrs.keys()) if isinstance(quad_attrs, dict) else 'N/A'}")
    ...
    if cd_micr_geo is None:
        logger.error(f"\n{'!'*80}")
        logger.error(f"❌ ERRO CRÍTICO: CD_MICR_GEO NÃO ENCONTRADO!")
        logger.error(f"{'!'*80}")
        logger.error(f"   ⚠️ CONSEQUÊNCIA: Notas agronômicas = 1.0 (CÁLCULOS INCORRETOS!)")
        logger.error(f"{'!'*80}\n")
    else:
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"🧮 CÁLCULOS DE VALORAÇÃO POR CLASSE")
    logger.info(f"{'='*80}")
    logger.info(f"   Fórmula: Valor = Área (ha) × Nota Agronômica × Valor do Quadrante")
    logger.info(f"{'='*80}\n")
    ...
    for cls_key, cls_info in relatorio["classes"].items():
        try:
            ...
            if cd_micr_geo is not None:
                logger.info(f"🔍 Buscando nota para: CD_MICR_GEO={cd_micr_geo}, Classe={cls_num}")
